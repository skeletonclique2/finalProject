...
def get_neighbors(adjacency_matrix, current):
    return adjacency_matrix.get(current, [])
...
for _, apartment in filtered_apartments.iterrows():
    ...
    intrusion_path.append((apartment['Floor'], apartment['Apartment']))

# Perform A* search for each end apartment
print("\nIntrusion Path:")
for end_apartment in intrusion_path:
    start_apartment = (1, 1)
    ...
    while open_set:
        ...
        for neighbor in get_neighbors(adjacency_matrix, current):
