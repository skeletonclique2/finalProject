...
# Perform A* search for each neighboring apartment of the intruded apartment
print("\nEvacuation Paths:")
for _, apartment in filtered_apartments.iterrows():
    intruded_apartment = (apartment['Floor'], apartment['Apartment'])
    neighboring_apartments = adjacency_matrix[intruded_apartment]

    for neighbor in neighboring_apartments:
        start_apartment = neighbor
        end_apartment = (1, 1)
        ...
        while open_set:
            ...
            if current == end_apartment:
                ...
                print(f"Evacuation Path from {intruded_apartment} to {end_apartment}:")
                for apt in path:
